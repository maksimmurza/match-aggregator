...
const hasNecessaryProperties = (responseObject: unknown, properties: Array<string>): boolean => {
  if (isObject(responseObject)) {
    return properties.every(property => property in responseObject && responseObject[property]);
  }
